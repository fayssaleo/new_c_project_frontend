...
export default {
  ...
  computed: {
    formTitle() {},
    ...mapGetters(["geteditedOrSavedClaimContainer","getLiability_letter_files_Data_Container"]),
    showDownload() {
  ...
  methods: {
    ...
    resetFiles() {
      this.dialog = true;
      var model = {
        container_id: this.geteditedOrSavedClaimContainer.id,
        type: "insurance",
      };

      this.setliability_letter_files_Data_containerAction(model).then(() => {
        this.insurance_declaration_files_DATA = [
          ...this.getLiability_letter_files_Data_Container,
        ];
      });
