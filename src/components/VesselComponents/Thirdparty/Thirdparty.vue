...
export default {
  ...
  computed: {
    ...
    ...mapGetters([
      ...
      "getdepartements",
      "geteditedOrSavedClaimVessel",
      "getLiability_letter_files_Data_Vessel",
    ]),
  },
  watch: {
    thirdparty: {
      ...
      handler(newValue, oldvalue) {
        this.set_thirdparty_vessel_claim_SetterAction(newValue).then(() => {});
      },
    },
  },
  created() {
  ...
  methods: {
    initialize() {
      if (this.geteditedOrSavedClaimVessel.id > 0) {
        var model = {
          vessel_id: this.geteditedOrSavedClaimVessel.id,
          type: "liability",
        };
        
        this.setliability_letter_files_Data_vesselAction(model).then(() => {
          this.liability_letter_files_DATA = [
            ...this.getLiability_letter_files_Data_Vessel,
          ];
        });
      }
    },
    ...
    ...mapActions([
      "set_thirdparty_vessel_claim_SetterAction",
