      ...
      <template>
        <v-container fluid>
          <v-row align="center">
            ...
            <v-col v-if="thirdparty.reinvoiced == 'YES'" class="d-flex" cols="12" sm="4">
              <v-text-field
  ...
  watch: {
    thirdparty: {
      ...
      handler(newValue, oldvalue) {
        this.set_reinvoiced_vessel_claim_SetterAction(newValue).then(() => {});
      },
    },
  },
  created() {
  ...
  methods: {
    initialize() {
      if (this.geteditedOrSavedClaimVessel.id > 0) {
        this.thirdparty.amount = this.geteditedOrSavedClaimVessel.amount;
        this.thirdparty.currency = this.geteditedOrSavedClaimVessel.currency;
        this.thirdparty.comment_third_party =
          this.geteditedOrSavedClaimVessel.comment_third_party;
        this.thirdparty.reinvoiced =
          this.geteditedOrSavedClaimVessel.reinvoiced;
        this.thirdparty.date_of_reimbursement =
          this.geteditedOrSavedClaimVessel.date_of_reimbursement;
        this.thirdparty.Invoice_number =
          this.geteditedOrSavedClaimVessel.Invoice_number;
        this.thirdparty.reimbursed_amount =
          this.geteditedOrSavedClaimVessel.reimbursed_amount;

      }
    },
    ...mapActions([
      "set_reinvoiced_vessel_claim_SetterAction",
    ]),
    clickOnChange() {
    },
